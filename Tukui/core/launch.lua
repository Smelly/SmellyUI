...
local function chatsetup()
	...
	for i = 1, NUM_CHAT_WINDOWS do
		...
		frame:Size(T.InfoLeftRightWidth + 1, 108)
		
		-- tell wow that we are using new size
		SetChatWindowSavedDimensions(chatFrameId, T.Scale(T.InfoLeftRightWidth + 1), T.Scale(108))
		
		-- move general bottom left or Loot (if found) on right
		if i == 1 then
			frame:ClearAllPoints()
			frame:Point("TOPLEFT", TukuiChatBackgroundLeft, "TOPLEFT", 8, 6)
		elseif i == 4 and chatName == LOOT then
			frame:ClearAllPoints()
			frame:Point("TOPRIGHT", TukuiChatBackgroundRight, "TOPRIGHT", -8, 6)
		end
...
sb:SetStatusBarTexture(C.media.blank)
